 ...
 *
 * The following log entries are from the merged files:
 *
 ...
 *
 * Version 1.22.7.CPP (August 20, 2005) (Arjen Wiersma)
 *      Upgraded syntax to Java 1.5 / 5.0 syntax, merging other grammar
 *      definitions into this C++ version (javaG.g / java1.5.g)
 *      * http://www.cenqua.com/clover/generics/javaG.g
 *      * find java1.5.g on the antlr homepage
...
protected typeDefinitionInternal[ANTLR_USE_NAMESPACE(antlr)RefAST modifiers]
    :   classDefinition[#modifiers]     // inner class
    |   interfaceDefinition[#modifiers] // inner interface
    |   enumDefinition[#modifiers]      // inner enum
    |   annotationDefinition[#modifiers]    // inner annotation
    ;
...
/**
 * Array declaration left bracket with everything until the right bracket
 */
arraySpecOpt:
 ...
 * - possible brackets afterwards (which would make it an array type).
...
/**
 * Class or Interface type which comes after the class specification
 * a non-built in type name, with possible type parameters
 */
classOrInterfaceType[bool addImagNode]
    :   IDENT (typeArguments)?
        (options{greedy=true;}: // match as many as possible
            DOT^
            IDENT (typeArguments)?
...
/**
 * Generics; classTypeSpecs that follow the class definition
 */
typeArguments
...
builtInTypeArraySpec[bool addImagNode]
...
modifiers
    :
        (
            ...
            {LA(1)==AT && (LT(2)->getText().compare("interface") != 0)}? annotation
        )*
...
enumDefinition![ANTLR_USE_NAMESPACE(antlr)RefAST modifiers]
    :   "enum" IDENT
        // it might implement some interfaces...
        ic:implementsClause
...
annotationDefinition![ANTLR_USE_NAMESPACE(antlr)RefAST modifiers]
    :   AT "interface" IDENT
...
/**
 * A generic type indicator can be a class or an interface
 */
typeParameter
    :
        (id:IDENT)
...
enumBlock
    :   LCURLY!
            ( enumConstant 
                ( 
                    options{greedy=true;}: COMMA! enumConstant 
                )* 
                ( COMMA! )? 
            )?
        ...
        |   // A generic method has the typeParameters before the return type.
            ...
            (   IDENT                                   // the name of the method
                ...
                {#enumConstantField = #(#[METHOD_DEF,"METHOD_DEF"],
                             mods, tp, #(#[TYPE,"TYPE"],rt), IDENT, param, tc, s2);}
            ...
            (
                ...
                |   // A generic method/ctor has the typeParameters before the return type.
                    ...
                    (   IDENT               // the name of the method
                        ...
                        ( s2:compoundStatement | SEMI )
                        {#classField = #(#[METHOD_DEF,"METHOD_DEF"], mods, tp,
                                     #(#[TYPE,"TYPE"],rt), IDENT, param, tc, s2);}
                    |   v:variableDefinitions[#mods,#t] SEMI
                        {#classField = #v;}
                    )
            )
    ...
    :   // method, constructor, or variable declaration
        ...
        |   (tp:typeParameters)?
            ...
            (   IDENT               // the name of the method
                ...
                {#interfaceField = #(#[METHOD_DEF,"METHOD_DEF"], mods, tp,
                             #(#[TYPE,"TYPE"],rt), IDENT, param, tc);}
            |   v:variableDefinitions[#mods,#t] SEMI
                {#interfaceField = #v;}
            )
        )
...
variableDefinitions[ANTLR_USE_NAMESPACE(antlr)RefAST mods, ANTLR_USE_NAMESPACE(antlr)RefAST t]
    :   variableDeclarator[getASTFactory()->dupTree(mods),
                           getASTFactory()->dupTree(t)] //dupList as this also copies siblings (like TYPE_ARGUMENTS)
        (   COMMA!
            variableDeclarator[getASTFactory()->dupTree(mods),
...
parameterDeclarationList
    ...
    :   (   ( parameterDeclaration )=> parameterDeclaration
        ...
        {#parameterDeclarationList = #(#[PARAMETERS,"PARAMETERS"], #parameterDeclarationList);}
    ;
...
unaryExpressionNotPlusMinus
    ...
    |   (   // use predicate to skip cases like: (int.class)
            // This subrule is used to make antlr not complain about
            // ambiguity
            options {
...
identPrimary
    :   (ta1:typeArguments!)?
        ...
        :   (   lp:LPAREN^ {#lp->setType(METHOD_CALL);}
                ...
                // FIXME: astFactory might not yet be available ;)
                {if (#ta2 != NULL) getASTFactory()->addASTChild(currentAST, #ta2);}
                {if (#ta2 == NULL) getASTFactory()->addASTChild(currentAST, #ta1);}
                argList RPAREN!
            )
        |   ( options {greedy=true;} :
